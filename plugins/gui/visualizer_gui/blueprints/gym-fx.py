...
def new_bp(plugin_folder, core_ep, store_ep, db, Base):
    ...
    def gymfx_validation_plot_():
        ...
        try:
            ...
            points = db.session.query(Base.classes.gym_fx_validation_plot).filter(Base.classes.gym_fx_validation_plot_fx_data.config_id == best ).order_by(desc(Base.classes.gym_fx_validation_plot.id)).limit(num_points).all()
