...
def list_data_index(db, Base, process, table, page_num=0, num_rows=25, filter_col=None, filter_val=None, order_by=None, asc_desc=None):
    try:
        ...
        if filter_col is None and order_by is None:
            res = db.session.query(Base.classes[table['name']]).offset(page_num*num_rows).limit(num_rows).all()
            ...
            res = db.session.query(Base.classes[table['name']]).filter(Base.classes[table['name']][filter_col] == filter_val).offset(page_num*num_rows).limit(num_rows).all()
            ...
            res = db.session.query(Base.classes[table['name']]).order_by(asc_desc(Base.classes[table['name']][order_by])).offset(page_num*num_rows).limit(num_rows).all()
        else:
